 ...
 * @returns
 */
export default async function sqlite_select(
  connection,
  table,
  target,
  where = null,
  ...
  try {
    ...
    else statement = connection.prepare(`SELECT ${target} FROM ${table}`);
    if (data) rows = statement.all(...data);
    else rows = statement.all();

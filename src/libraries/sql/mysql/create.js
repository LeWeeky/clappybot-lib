 ...
 * @returns
 */
export default async function mysql_create_table(connection, name, content, more = null) {
  try {
    if (more)
      await connection
        .promise()
        .query(`CREATE TABLE IF NOT EXISTS ${name} (${content}) ${more}`);
    else
      await connection
        .promise()
        .query(`CREATE TABLE IF NOT EXISTS ${name} (${content})`);

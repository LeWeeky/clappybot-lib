...
export default class DataBaseWrapper {
  ...
  #databases: { [key: string]: ADriver } = {};

  constructor() {}

  add<T extends ADriver>(driver: T, name: string) {
    this.#databases[name] = driver;
  }

  remove(driver_name: string) {
    if (!this.#databases[driver_name]) return false;
    this.#databases[driver_name].destroy();
    delete this.#databases[driver_name];
    return true;
  }

  select(driver_name: string): ADriver | null {
    if (this.#databases[driver_name]) return this.#databases[driver_name];
    return null;
  }
